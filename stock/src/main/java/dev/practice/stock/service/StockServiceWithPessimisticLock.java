...
public class StockServiceWithPessimisticLock {
     ...
     * - 만약, PK 가 아닌 다른 컬럼으로 조회를 한다면, Table 단위로 락이 걸릴수 있으며, 인덱스를 사용했다고 해도 동일한 인덱스 값에 대해 락이 걸리므로
     * - 불필요한 row 까지 락이 걸릴 수 있다.
     ...
     * 7. Mutex, synchronized 키워드, ReentrantLock 와 같은 방식이다.
     * 8. ReentrantReadWriteLock 은 공유락 과 배타락을 동시에 사용할 수 있고 개념상 비관적 락이다.
     */
    @Transactional
